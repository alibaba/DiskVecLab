...
QueryEngine::~QueryEngine() {
    ...
    if(parallel and nprobes > 1){
        for(int i=1;i<nworker;i++){
            ...
            delete qwdata[i].visited_leaves;
...
QueryEngine::QueryEngine(const char *query_filename, Index *index, int ef, unsigned int nprobes, bool parallel,
    ...
    if(parallel and nprobes >1){
        ...
        for(int i=1;i<this->nworker;i++){
            ...
            qwdata[i].visited_leaves = new std::unordered_set<unsigned int>();
...
//        qwdata[0].localknn = static_cast<float *>(malloc(k*sizeof(float)));
        ...
        qwdata[0].visited_leaves = &unique_visited_leaves;
...
void QueryEngine::queryOne(ts_type *query_ts, unsigned int k) {
    for(int i=1;i<this->nworker;i++){
        qwdata[i].visited_leaves->clear();
    }
    unique_visited_leaves.clear();
    searchNpLeafParallel(query_ts,k,nprobes);
}

const std::unordered_set<unsigned int>& QueryEngine::get_unique_visited_leaves() {
    return unique_visited_leaves;
}
...
void QueryEngine::queryBinaryFile(int q_num, unsigned int k, int mode) {
    ...
    std::string split_path(this->index->index_setting->split_path);
    std::string original_id_filename = split_path + "/_selected_ids_uint32.bin";
    FILE* original_id_file = fopen(original_id_filename.c_str(), "wb");
    if (original_id_file != nullptr) {
        unsigned int total = unique_visited_leaves.size();
        fwrite(&(total), sizeof(uint32_t), 1, original_id_file);
        uint32_t dim = 1;
        fwrite(&dim, sizeof(uint32_t), 1, original_id_file);

        for (auto it = unique_visited_leaves.begin(); it != unique_visited_leaves.end(); ++it) {
            unsigned int leaf_id = *it;
            fwrite(&leaf_id, sizeof(unsigned int), 1, original_id_file);
        }
        fclose(original_id_file);
    } else {
        cerr << "Failed to open leaf ID file: " << original_id_filename << endl;
    }
...
unsigned int ex_leaf_id(Node * node) {
    // Extract the ID from the filename
    unsigned int filename_id = static_cast<unsigned int>(node->id); // Default to node->id
    if (node->filename != nullptr) {
        // Parse the ID from the filename (last part after the final underscore)
        char* last_underscore = strrchr(node->filename, '_');
        if (last_underscore != nullptr) {
            filename_id = static_cast<unsigned int>(strtoul(last_underscore + 1, nullptr, 10));
        }
    }
    return filename_id;
}
...
unsigned int QueryEngine::get_first_leaf(ts_type *query_ts) {
    stats.reset();
    ...
    set_app_node(App_node);
    return ex_leaf_id(App_node);
}

void QueryEngine::set_app_node(Node * node) {
    this->App_node = node;
}

void QueryEngine::get_other_leaves(ts_type *query_ts, unsigned int k, float distance) {
    get_other_leaves_impl(query_ts, k, nprobes, distance);
}

void QueryEngine::get_other_leaves_impl(ts_type *query_ts, unsigned int k, unsigned int nprobes, float distance) {
    ts_type kth_bsf = distance;
    nprobes--;
    if (nprobes == 0) {
        while (top_candidates.size() > k)top_candidates.pop();
...
#pragma omp for schedule(static, 1)
                for (int i = 0; i < std::min(candidates_count,nprobes); i++) {
                    ...
                    if (node.distance <=worker->bsf) {
                        ...
                        unsigned int leaf_id = ex_leaf_id(node.node);
                        worker->visited_leaves->insert(leaf_id);
...
//#pragma omp for schedule(static, 1)
        ...
        for(int i=1;i<nworker;i++){
            ...
            unique_visited_leaves.insert(qwdata[i].visited_leaves->begin(),qwdata[i].visited_leaves->end());
            //for(int j = 0;j<k;j++){
...
void QueryEngine::searchNpLeafParallel(ts_type *query_ts, unsigned int k, unsigned int nprobes) {
    ...
    unique_visited_leaves.insert(ex_leaf_id(App_node));
