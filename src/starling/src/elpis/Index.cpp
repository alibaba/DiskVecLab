...
Index * Index::initIndex(char *path, unsigned int size, double size1, unsigned int segments, unsigned int size2,
                         int construction,
                         int m, char *split_path) {

    return new Index(path, size, size1, segments, size2, construction, m, split_path);

}

Index::Index(char *index_path, unsigned int timeseries_size, double buffered_memory_size,
             unsigned int init_segments, unsigned int max_leaf_size, int efConstruction, int M, char *split_path) {
    ...
    char * index_path_hercules = static_cast<char *>(malloc(sizeof(char) * (strlen(index_path) + 5)));
    index_path_hercules = strcpy( index_path_hercules , index_path);
    index_path_hercules = strcat( index_path_hercules , "tmp/");
    index_path_hercules = strcat(index_path_hercules, "\0");
    mkdir(index_path_hercules,0777);

    cout << index_path_hercules << endl;

    this->index_setting =new Setting(index_path_hercules,index_path, timeseries_size, init_segments, max_leaf_size, buffered_memory_size, efConstruction, M, split_path);
...
void Index::exportData(Node* node) {
    std::string split_path(this->index_setting->split_path);
    if(node->is_leaf) {
        cout << "[EXPORT] Exporting data for leaf node " << node->id << " with size " << node->node_size << " in file " << node->filename << endl;
        
        // write raw data
        std::string raw_data_filename = split_path + "/_subshard-" + std::to_string(node->id) + ".bin";
        FILE* raw_file = fopen(raw_data_filename.c_str(), "wb");
        if(raw_file != nullptr) {
            fwrite(&(node->node_size), sizeof(uint32_t), 1, raw_file);
            uint32_t dim = this->index_setting->timeseries_size;
            fwrite(&(dim), sizeof(uint32_t), 1, raw_file);
            ts_type **timeseries_data = node->getTS(this);
            if(timeseries_data != nullptr) {
                for(unsigned int i = 0; i < node->node_size; i++) {
                    fwrite(timeseries_data[i], sizeof(ts_type), this->index_setting->timeseries_size, raw_file);
                }
            }
            free(timeseries_data);
            fclose(raw_file);
        } else {
            cerr << "[EXPORT] Failed to open raw data file: " << raw_data_filename << endl;
        }
    }
}
...
Index *Index::Read(char *path, unsigned int mode, char *split_path) {
    return new Index(path, mode, split_path);
}
Index::Index(char *root_directory, unsigned int mode, char *split_path) {
...
cerr << index_full_filename << endl;
    ...
    this -> index_setting = new Setting(root_directory, root_directory, timeseries_size, init_segments,
                                        max_leaf_size, buffered_memory_size, -1, -1, split_path);
