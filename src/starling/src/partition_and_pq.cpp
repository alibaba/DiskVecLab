...
int generate_rabitq_data(const float *train_data,const std::string data_file_path,
                         const std::string &index_file_path, int64_t dim, int64_t num_points,
                         const std::string& data_type) {
  auto pq_table = PQFactory::create_pq_table(PQType::RABITQ);
...
int generate_lsq_data(const float *train_data, const std::string data_file_path,
                      ...
                      int64_t dim, int64_t num_points,
                      const std::string& data_type) {
  auto pq_table = PQFactory::create_pq_table(PQType::LSQ);
  pq_table->train(num_points, train_data, data_file_path, index_file_path, data_type);
  return 0;
}
                                              ...
                                              const double p_val, const size_t num_pq_chunks, const bool use_opq)
{
  ...
  switch (pq_type) {
    case PQType::PQ:
      diskann::cout << "Training PQ index" << std::endl;
      ...
      break;
    case PQType::RABITQ:
      diskann::cout << "Training RabitQ index" << std::endl;
      generate_rabitq_data(train_data, data_file_to_use, disk_rabitq_index_path, (uint32_t)train_dim, train_size, data_type_str);
    case PQType::LSQ:
      diskann::cout << "Training LSQ index" << std::endl;
      generate_lsq_data(train_data, data_file_to_use, disk_lsq_index_path, (uint32_t)train_dim, train_size, data_type_str);
      delete[] train_data;
      break;
    case PQType::OPQ:
      // not supported for now
      throw std::runtime_error("Unsupported OPQ");
      break;
    default:
