...
// Debug control: enable detailed output for first query
static std::atomic<bool> g_first_query_debug{true};
  template<typename T>
  ...
  void PQFlashIndex<T>::page_search(
    ...
    // query <-> neighbor list
    std::unique_ptr<DistanceComputer> dc_computer(pq_table->preprocess_query(query_float));
...
//    };
    ...
    auto compute_and_push_nbrs = [&](const char *node_buf, unsigned& nk) {
      ...
      if (nbors_cand_size) {
        pq_table->compute_dists(node_nbrs, nbors_cand_size, dist_scratch, this->data, pq_coord_scratch, pq_dists, dc_computer.get());
    ...
    auto compute_and_add_to_retset = [&](const unsigned *node_ids, const _u64 n_ids) {
      pq_table->compute_dists(node_ids, n_ids, dist_scratch, this->data, pq_coord_scratch, pq_dists, dc_computer.get());
    ...
    // Debug output for first query (baseline mode)
    static std::atomic<bool> g_baseline_first_query{true};
    bool is_first_query = g_baseline_first_query.exchange(false);
    if (is_first_query) {
      std::cout << "\n========== FIRST QUERY DEBUG (page_search BASELINE) ==========" << std::endl;
      std::cout << "L=" << l_search << ", beam_width=" << beam_width 
                << ", io_limit=" << io_limit << std::endl;
      std::cout << "Initial retset size: " << cur_list_size << std::endl;
      std::cout << "Starting search loop..." << std::endl;
    }
    ...
    while (k < cur_list_size && num_ios < io_limit) {
      if (is_first_query) {
        ...
        std::cout << "k=" << k << ", cur_list_size=" << cur_list_size 
                  << ", num_ios=" << num_ios << std::endl;
        std::cout << "Top-5 in retset: ";
        for (unsigned i = 0; i < std::min(5u, cur_list_size); i++) {
          std::cout << "[id=" << retset[i].id << ",dist=" << retset[i].distance 
                    << ",flag=" << (retset[i].flag?"T":"F") << "] ";
        }
        std::cout << std::endl;
      }
      ...
      if (is_first_query) {
        std::cout << "Frontier nodes to visit: ";
        for (auto node_id : frontier) {
          std::cout << node_id << "(page=" << id2page_[node_id] << ") ";
        }
        std::cout << std::endl;
        std::cout << "Cached nodes: " << cached_nhoods.size() << std::endl;
      }
      ...
      if (!frontier.empty()) {
        ...
        if (is_first_query) {
          std::cout << "IO batch size: " << frontier.size() 
                    << ", total IOs so far: " << num_ios << std::endl;
        }
...
#pragma omp critical
    ...
    if (is_first_query) {
      std::cout << "\n========== SEARCH COMPLETE (BASELINE) ==========" << std::endl;
      std::cout << "Total iterations: " << iteration << std::endl;
      std::cout << "Total IOs: " << num_ios << std::endl;
      std::cout << "Final retset size: " << cur_list_size << std::endl;
      std::cout << "Total pages visited: " << page_visited.size() << std::endl;
      std::cout << "Total nodes visited: " << visited.size() << std::endl;
      std::cout << "===============================================\n" << std::endl;
    }
...
#pragma omp critical
  ...
  // ============================================================================
  // Incremental Page Search Implementation
  // ============================================================================

  template<typename T>
  void PQFlashIndex<T>::init_page_search(
      const T *query1, const _u64 k_search, const _u32 mem_L, 
      const _u64 l_capacity, const _u64 l_stop,
      const _u64 beam_width, const _u32 io_limit, const float use_ratio,
      SearchState<T> &state, QueryStats *stats) {
    ...
    state.initialize(l_capacity, l_stop);
    ...
    // Use state's data structures
    std::vector<Neighbor> &retset = state.retset();
    unsigned &cur_list_size = state.cur_list_size();
    tsl::robin_set<uint64_t> &visited = state.visited();
    tsl::robin_set<unsigned> &page_visited = state.page_visited();
    std::vector<Neighbor> &full_retset = state.full_retset();
    ...
    auto compute_extact_dists_and_push = [&](const char *node_buf, const unsigned id) -> float {
      ...
      float cur_expanded_dist = dist_cmp->compare(query, node_fp_coords_copy,
                                                  (unsigned)aligned_dim);
      full_retset.push_back(Neighbor(id, cur_expanded_dist, true));
      // Note: do NOT add exact distance to _all_candidates.
      // _all_candidates only tracks PQ distances for consistency with retset.
      return cur_expanded_dist;
    };

    // Insertion uses l_capacity (retset capacity), not l_stop
    auto compute_and_push_nbrs = [&](const char *node_buf, unsigned &nk) {
      unsigned *node_nbrs = OFFSET_TO_NODE_NHOOD(node_buf);
      unsigned nnbrs = *(node_nbrs++);
      unsigned nbors_cand_size = 0;
      ...
      if (nbors_cand_size) {
        pq_table->compute_dists(node_nbrs, nbors_cand_size, dist_scratch, this->data,
                                pq_coord_scratch, pq_dists, dc_computer.get());
        for (unsigned m = 0; m < nbors_cand_size; ++m) {
          const int nbor_id = node_nbrs[m];
          const float nbor_dist = dist_scratch[m];
          state.add_cmps(1);
          Neighbor nn(nbor_id, nbor_dist, true);
          // Use l_capacity for insertion: retset always has full capacity
          if (nbor_dist >= retset[cur_list_size - 1].distance &&
              (cur_list_size == l_capacity))
            continue;
          auto r = InsertIntoPool(retset.data(), cur_list_size, nn);
          if (cur_list_size < l_capacity)
            ++cur_list_size;
          if (r < nk)
            nk = r;
        }
    ...
    auto compute_and_add_to_retset = [&](const unsigned *node_ids, const _u64 n_ids) {
      pq_table->compute_dists(node_ids, n_ids, dist_scratch, this->data,
                              pq_coord_scratch, pq_dists, dc_computer.get());
      for (_u64 i = 0; i < n_ids; ++i) {
        retset[cur_list_size].id = node_ids[i];
        retset[cur_list_size].distance = dist_scratch[i];
    ...
    // Initialize with mem_index or medoid
    if (mem_L) {
      std::vector<unsigned> mem_tags(mem_L);
      std::vector<float> mem_dists(mem_L);
      std::vector<T *> res = std::vector<T *>();
      mem_index_->search_with_tags(query, mem_L, mem_L, mem_tags.data(), mem_dists.data(), nullptr, res);
      compute_and_add_to_retset(mem_tags.data(), std::min((unsigned)mem_L, (unsigned)l_capacity));
    } else {
      compute_and_add_to_retset(&best_medoid, 1);
    }
    ...
    bool is_first_query = g_first_query_debug.exchange(false);
    if (is_first_query) {
      std::cout << "\n========== FIRST QUERY DEBUG (init_page_search) ==========" << std::endl;
      std::cout << "L_capacity=" << l_capacity << ", L_stop=" << l_stop
    ...
    // Search loop: k only advances up to l_stop (stopping condition),
    // but retset accepts candidates up to l_capacity (insertion capacity).
    unsigned l_stop_bound = std::min((unsigned)l_stop, cur_list_size);
    unsigned iteration = 0;
    while (k < l_stop_bound && num_ios < io_limit) {
      // Update bound as cur_list_size may grow during search
      l_stop_bound = std::min((unsigned)l_stop, cur_list_size);
      unsigned nk = cur_list_size;
      frontier.clear();
      frontier_nhoods.clear();
      frontier_read_reqs.clear();
      ...
      // Marker only scans up to l_stop_bound: nodes beyond l_stop are in
      // retset but won't be expanded until l_stop is increased on resume.
      while (marker < l_stop_bound && frontier.size() < beam_width &&
             num_seen < beam_width) {
        const unsigned pid = id2page_[retset[marker].id];
        if (page_visited.find(pid) == page_visited.end() && retset[marker].flag) {
          num_seen++;
      ...
      if (!frontier.empty()) {
        ...
        n_ops = reader->submit_reqs(frontier_read_reqs, ctx);
      } else if (is_first_query) {
        std::cout << "No new nodes to visit (frontier empty)" << std::endl;
      }
      ...
      // Update l_stop_bound in case cur_list_size changed
      l_stop_bound = std::min((unsigned)l_stop, cur_list_size);
    }

    // Persist deferred page data to state so that resume_page_search can
    // process them in its first iteration (overlapping with new I/O),
    // exactly matching the original page_search pipeline.
    state.last_io_ids() = std::move(last_io_ids);
    state.last_pages() = std::move(last_pages);

    // Save search cursor for potential resume
    state.set_search_cursor(k);
    ...
    if (is_first_query) {
      ...
      std::cout << "Total IOs: " << num_ios << std::endl;
      std::cout << "Final retset size: " << cur_list_size << std::endl;
      std::cout << "L_capacity: " << l_capacity << ", L_stop: " << l_stop << std::endl;
      std::cout << "Search cursor saved at k=" << k << std::endl;
      std::cout << "Total pages visited: " << page_visited.size() << std::endl;
      std::cout << "Total nodes visited: " << visited.size() << std::endl;
      std::cout << "====================================\n" << std::endl;
    }
  ...
  template<typename T>
  void PQFlashIndex<T>::resume_page_search(
      const T *query1, const _u64 k_search, const _u32 mem_L, const _u64 new_l_stop,
      const _u64 beam_width, const _u32 io_limit, const float use_ratio,
      SearchState<T> &state, QueryStats *stats) {

    if (!state.is_initialized()) {
      throw ANNException("SearchState not initialized. Call init_page_search first.",
                         -1, __FUNCSIG__, __FILE__, __LINE__);
    }

    // Update stopping point (no capacity change — retset is already large enough)
    state.set_l_stop(new_l_stop);

    // If no more unexpanded nodes AND no deferred pages to process, nothing to do
    if (!state.has_unexpanded_node() && state.last_io_ids().empty()) {
      return;
    }
    ...
    tsl::robin_set<unsigned> &page_visited = state.page_visited();
    std::vector<Neighbor> &full_retset = state.full_retset();
    uint64_t l_capacity = state.get_l_capacity();
    uint64_t l_stop = state.get_l_stop();
    ...
    // Use cumulative IO count from state to enforce total IO budget
    // across init_page_search + all resume_page_search calls
    unsigned cumulative_ios = state.get_num_ios();

    // Restore search cursor from where we paused
    unsigned k = state.get_search_cursor();
    ...
    // Restore deferred page data from state — these are the leftover pages
    // from the previous init/resume call that haven't been processed yet.
    // The first loop iteration will process them while new I/O is in flight,
    // exactly matching the original page_search pipeline.
    std::vector<unsigned> last_io_ids = std::move(state.last_io_ids());
    std::vector<char> last_pages = std::move(state.last_pages());
    // Ensure last_pages has enough capacity for new pages in this call
    size_t required_pages_size = SECTOR_LEN * beam_width * 2;
    if (last_pages.size() < required_pages_size) {
      last_pages.resize(required_pages_size);
    }
    int n_ops = 0;

    // Search loop: k advances up to l_stop (stopping condition).
    // Note: We also allow entering the loop when there are deferred pages
    // (last_io_ids) even if k has already reached l_stop_bound, so that we can
    // drain deferred work without issuing new I/O. This matches the original
    // page_search pipeline behavior where deferred pages are processed in the
    // next iteration.
    unsigned l_stop_bound = std::min((unsigned)l_stop, cur_list_size);
    while ((k < l_stop_bound || !last_io_ids.empty()) && cumulative_ios < io_limit) {
      // Update bound as cur_list_size may grow during search
      l_stop_bound = std::min((unsigned)l_stop, cur_list_size);
      ...
      // Marker only scans up to l_stop_bound
      while (marker < l_stop_bound && frontier.size() < beam_width &&
             num_seen < beam_width) {
        const unsigned pid = id2page_[retset[marker].id];
        // flag=true means unexpanded; page_visited prevents re-fetching same page
        if (page_visited.find(pid) == page_visited.end() && retset[marker].flag) {
          num_seen++;
          auto iter = nhood_cache.find(retset[marker].id);
          if (iter != nhood_cache.end()) {
    ...
    // Persist deferred page data to state for the next resume call
    state.last_io_ids() = std::move(last_io_ids);
    state.last_pages() = std::move(last_pages);

    // Save search cursor for potential next resume
    state.set_search_cursor(k);
  ...
  template<typename T>
  void PQFlashIndex<T>::get_search_results(
      const _u64 k_search, SearchState<T> &state,
