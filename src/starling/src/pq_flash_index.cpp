...
#else
  template<typename T>
  int PQFlashIndex<T>::load(uint32_t num_threads, const char *index_prefix, const std::string& disk_index_path, PQType pq_type) {
#endif
    ...
    std::string pq_index_file;
    if (pq_type == PQType::LSQ) {
      pq_index_file = std::string(index_prefix) + "_faiss_lsq.index";
    } else if (pq_type == PQType::RABITQ) {
      pq_index_file = std::string(index_prefix) + "_faiss_rabitq.index";
    }
    pq_table = PQFactory::create_pq_table(pq_type);
    ...
    {
      pq_table->load_pq_compressed_vectors(pq_index_file, this->data);
      this->num_points = pq_table->get_num_points();
    } else {
#ifdef EXEC_ENV_OLS
...
#pragma omp critical
      ...
      for (auto &cached_nhood : cached_nhoods) {
        ...
        cpu_timer.reset();
        pq_table->compute_dists(node_nbrs, nnbrs, dist_scratch, this->data, pq_coord_scratch, pq_dists, dc_computer.get());
        if (stats != nullptr) {
