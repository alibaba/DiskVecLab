...
namespace diskann {
  ...
  size_t get_pq_num_chunks(const std::string &mem_budget_str) {
    if (mem_budget_str.empty()) {
      std::stringstream stream;
      stream << "empty string. " << std::endl;
      throw diskann::ANNException(stream.str(), -1);
    }

    unsigned long long value = std::stoull(mem_budget_str);
    if (value > SIZE_MAX) {
      std::stringstream stream;
      stream << "value too large for size_t. " << std::endl;
      throw diskann::ANNException(stream.str(), -1);
    }
  ...
  double calculate_recall(unsigned num_queries, unsigned *gold_std,
    ...
    // diskann::cout << "Total recall : " << total_recall << std::endl;
...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
  ...
  int build_disk_index(const char *dataFilePath, const char *indexFilePath,
    ...
    std::string rabitq_index_path = index_prefix_path + "_faiss_rabitq.index";
    std::string lsq_index_path = index_prefix_path + "_faiss_lsq.index";
    ...
    size_t num_pq_chunks = get_pq_num_chunks(param_list[2]);
    diskann::cout << "num_pq_chunks: " << num_pq_chunks << std::endl;
    ...
    generate_quantized_data_build_disk_index<T>(data_file_to_use, pq_pivots_path, pq_compressed_vectors_path, lsq_index_path,
                                                rabitq_index_path, compareMetric, p_val,
