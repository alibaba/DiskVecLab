...
int search_disk_index(
    ...
    const _u32 mem_L,
    std::string quantification_type = "PQ",
  ...
  int res = _pFlashIndex->load(num_threads, index_path_prefix.c_str(), disk_file_path, pq_type);
  ...
  try {
    if (data_type == std::string("float"))
      return search_disk_index<float>(metric, index_path_prefix,
                                      ...
                                      search_io_limit, Lvec, mem_L, quantification_type, use_page_search, use_ratio, use_reorder_data, use_sq);
      ...
      return search_disk_index<int8_t>(metric, index_path_prefix,
                                       ...
                                       disk_file_path,
                                       num_threads, K, W, num_nodes_to_cache,
                                       search_io_limit, Lvec, mem_L, quantification_type, use_page_search, use_ratio, use_reorder_data);
    else if (data_type == std::string("uint8"))
      return search_disk_index<uint8_t>(
          metric, index_path_prefix, mem_index_path, result_path_prefix, query_file, gt_file,
          disk_file_path, num_threads, K, W, num_nodes_to_cache, search_io_limit, Lvec, mem_L, quantification_type,
