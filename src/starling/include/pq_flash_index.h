  ...
  class PQFlashIndex {
   public:
    ...
    DISKANN_DLLEXPORT int load(diskann::MemoryMappedFiles &files,
                               uint32_t num_threads, const char *index_prefix, PQType pq_type = PQType::PQ);
#else
    // load compressed data, and obtains the handle to the disk-resident index
    DISKANN_DLLEXPORT int  load(uint32_t num_threads, const char *index_prefix,
        const std::string& disk_index_path, PQType pq_type = PQType::PQ);
#endif
    ...
    /**
     * Initialize an incremental page search.
     * The retset is sized to l_capacity (fixed), while the search cursor only
     * advances up to l_stop before pausing.
     * 
     * @param query Query vector
     * @param k_search Number of results to return
     * @param mem_L L parameter for memory graph (0 to disable)
     * @param l_capacity Fixed retset capacity (candidate queue size)
     * @param l_stop Initial stopping point for search cursor
     * @param beam_width Beam width for IO batching
     * @param io_limit Maximum number of IO operations
     * @param use_ratio Page search use ratio
     * @param state Output search state (will be initialized)
     * @param stats Optional statistics output
     */
    DISKANN_DLLEXPORT void init_page_search(
        ...
        SearchState<T> &state, QueryStats *stats = nullptr);

    /**
     * Resume a previously paused page search with an increased stopping point.
     * The retset capacity stays the same; only the search boundary is extended.
     * The search continues from the saved cursor position.
     * 
     * @param query Query vector (must be the same as init_page_search)
     * @param k_search Number of results to return
     * @param mem_L L parameter for memory graph
     * @param new_l_stop New stopping point for search cursor
     * @param beam_width Beam width for IO batching
     * @param io_limit Maximum IO operations (cumulative across all calls)
     * @param use_ratio Page search use ratio
     * @param state Search state from init_page_search or previous resume_page_search
     * @param stats Optional statistics output (accumulates with previous stats)
     */
    DISKANN_DLLEXPORT void resume_page_search(
        const T *query, const _u64 k_search, const _u32 mem_L, const _u64 new_l_stop,
        const _u64 beam_width, const _u32 io_limit, const float use_ratio,
        SearchState<T> &state, QueryStats *stats = nullptr);

    /**
     * Get results from a search state.
     * Can be called after init_page_search or resume_page_search.
     * 
     * @param k_search Number of results to return
     ...
     */
    DISKANN_DLLEXPORT void get_search_results(
        const _u64 k_search, SearchState<T> &state,
        _u64 *res_ids, float *res_dists);

        // Expose node -> page mapping for multi-shard schedulers.
        // Valid only after load_partition_data() has been called during index load.
        DISKANN_DLLEXPORT inline unsigned get_page_id(unsigned node_id) const {
            return this->id2page_[node_id];
        }
   ...
   private:
    ...
    _u8 *             data = nullptr;
    _u64              n_chunks = 0;
    std::unique_ptr<PQTableBase> pq_table;
