...
public:
    ...
    Index(char *root_directory, unsigned int mode, char *split_path = nullptr);
    ~Index();
    ...
    static Index *Read(char *path, unsigned int mode, char *split_path = nullptr);
...
protected :
    Index(char *index_path, unsigned int time_series_size, double buffered_memory_size, unsigned int init_segments,
          unsigned int leaf_size, int efConstruction, int M, char *split_path = nullptr);
