...
public :
    ...
    FILE *query_file;
    std::unordered_set<unsigned int> unique_visited_leaves;
    Node * App_node;
    ...
    void get_other_leaves(ts_type *query_ts, unsigned int k, float distance);
    void get_other_leaves_impl(ts_type *query_ts, unsigned int k, unsigned int nprobes, float distance);
    unsigned int get_first_leaf(ts_type *query_ts);
    void set_app_node(Node * node);
    const std::unordered_set<unsigned int>& get_unique_visited_leaves();
    void queryOne(ts_type *query_ts, unsigned int k);
...
typedef struct query_worker_data
{
    ...
    unsigned short *flags;
    unsigned short curr_flag;
    std::unordered_set<unsigned int> * visited_leaves;
