...
namespace SPTAG
{
    ...
    namespace SPANN
    {
        ...
        void Index<T>::SetQuantizer(std::shared_ptr<SPTAG::COMMON::IQuantizer> quantizer)
        {
            ...
            if (m_pQuantizer)
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "set quantizer in spann 44\n");
            ...
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "set no quantizer in spann 50\n");
            ...
            if (m_index)
            {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "set quantizer in spann 55\n");
            ...
            if (m_extraSearcher != nullptr) {
                ...
                for (int i = 0; i < p_queryResults->GetResultNum(); ++i)
                {
                    ...
                    if (workSpace->m_postingIDs.size() >= m_options.m_searchInternalResultNum ||
                        ...
                        continue;
                    //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "postingID = %d", postingID);
        ...
        ErrorCode Index<T>::SearchDiskIndex(QueryResult& p_query, SearchStats* p_stats) const
        {
            ...
            for (; i < m_options.m_searchInternalResultNum; ++i)
            {
                ...
                if (m_extraSearcher->CheckValidPosting(res->VID)) 
                {
                    ...
                    //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "postingID = %d", res->VID);
                }
                res->VID = static_cast<SizeType>((m_vectorTranslateMap.get())[res->VID]);
                if (res->VID == MaxSize) 
                {
...
#pragma endregion
        ...
        bool Index<T>::SelectHeadInternal(std::shared_ptr<Helper::VectorSetReader>& p_reader) {
            std::shared_ptr<VectorSet> vectorsetBkt = p_reader->GetVectorSet();
            if (m_options.m_distCalcMethod == DistCalcMethod::Cosine && !p_reader->IsNormalized())
                vectorsetBkt->Normalize(m_options.m_iSelectHeadNumberOfThreads);


            COMMON::Dataset<InternalDataType> data(vectorsetBkt->Count(), vectorsetBkt->Dimension(), vectorsetBkt->Count(), vectorsetBkt->Count() + 1, (InternalDataType*)vectorsetBkt->GetData());
            SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Begin initial data (%d,%d)...\n", vectorsetBkt->Count(), vectorsetBkt->Dimension());
            ...
            if (data.R() == 1) {
                ...
                //auto saved_rg_state = rg;
                //int saved_rand_state = std::rand();  // Get current rand() state
                
                // Reset both RNGs to fixed seeds for consistent head selection
                //rg.seed(12345);     // Fixed seed for std::shuffle operations
                //std::srand(54321);  // Fixed seed for COMMON::Utils::rand operations
                ...
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Start invoking BuildTrees with fixed seed for consistent head selection.\n");
                ...
                // Restore RNG states
                //rg = saved_rg_state;
                // Note: std::rand() state cannot be easily restored, but this is acceptable
                // since we're only ensuring consistency within head selection
            ...
            if (!m_options.m_noOutput)
            {
                ...
                // Write dimension based on whether quantizer is used
                //DimensionType dt = m_pQuantizer ? m_pQuantizer->GetNumSubvectors() : data.C();
                ...
                for (int i = 0; i < selected.size(); i++)
                {
                    ...
                    if (m_pQuantizer) {
                        // Quantize the original full precision vector and write quantized data
                        /*std::vector<uint8_t> quantizedVec(m_pQuantizer->QuantizeSize());
                        m_pQuantizer->QuantizeVector(data[vid], quantizedVec.data(), false);
                        if (output->WriteBinary(quantizedVec.size(), (char*)quantizedVec.data()) != quantizedVec.size()) {
                            SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to write quantized vector!\n");
                            return false;
                        }*/
        ...
        ErrorCode Index<T>::BuildIndexInternal(std::shared_ptr<Helper::VectorSetReader>& p_reader) {
            ...
            // Determine which vector path and type to use for BKT construction
            std::string fullVectorPathToUse;
            VectorFileType fullVectorTypeToUse;
            
            if (!m_options.m_fullVectorPath.empty()) {
                // Use explicitly specified full precision vector path and type
                fullVectorPathToUse = m_options.m_fullVectorPath;
                fullVectorTypeToUse = m_options.m_fullVectorType;
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Using full precision vectors from FullVectorPath: %s (type: %s)\n", 
                    fullVectorPathToUse.c_str(), 
                    Helper::Convert::ConvertToString(fullVectorTypeToUse).c_str());
            } else if (m_pQuantizer) {
                // If quantizer is present but no full vector path specified, this is an error
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Quantizer is present but FullVectorPath is not specified in config!\n");
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Please add 'FullVectorPath=<path_to_full_precision_vectors>' in [Base] section of config file.\n");
                return ErrorCode::Fail;
            } else {
                // No quantizer, use the regular vector path and type
                fullVectorPathToUse = m_options.m_vectorPath;
                fullVectorTypeToUse = m_options.m_vectorType;
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "No quantizer present, using VectorPath for BKT construction: %s (type: %s)\n", 
                    fullVectorPathToUse.c_str(),
                    Helper::Convert::ConvertToString(fullVectorTypeToUse).c_str());
            }

            // Load original full precision vectors for BKT construction
            std::shared_ptr<Helper::ReaderOptions> vectorOptionsBkt(new Helper::ReaderOptions(m_options.m_valueType, m_options.m_dim, fullVectorTypeToUse, m_options.m_vectorDelimiter));
            auto vectorReaderBkt = Helper::VectorSetReader::CreateInstance(vectorOptionsBkt);
            if (ErrorCode::Success == vectorReaderBkt->LoadFile(fullVectorPathToUse))
            {
                auto vectorsetBkt = vectorReaderBkt->GetVectorSet();
                if (m_options.m_distCalcMethod == DistCalcMethod::Cosine && !vectorReaderBkt->IsNormalized()) {
                    vectorsetBkt->Normalize(m_options.m_iSelectHeadNumberOfThreads);
                }
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Load full precision VectorSet for BKT construction (%d,%d).\n", vectorsetBkt->Count(), vectorsetBkt->Dimension());
            } else {
                SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Failed to load full precision vectors from %s\n", fullVectorPathToUse.c_str());
                return ErrorCode::Fail;
            }
            if (m_options.m_selectHead) {
                ...
                if (m_pQuantizer)
                {
                    success = SelectHeadInternal<T>(vectorReaderBkt);
                }
                else
                {
            ...
            if (m_options.m_buildHead) {
                ...
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "index algo type %d\n", m_options.m_indexAlgoType);
            ...
            if (m_options.m_enableSSD) {
                ...
                if (m_options.m_buildSsdIndex) {
                    ...
                    if (!m_extraSearcher->BuildIndex(vectorReaderBkt, p_reader, m_index, m_options)) {
