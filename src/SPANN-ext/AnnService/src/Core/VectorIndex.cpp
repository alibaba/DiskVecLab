...
VectorIndex::LoadQuantizer(std::string p_quantizerFile)
{
    ...
    SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "LoadQuantizer from file: %s, loaded quantizer at %p\n", p_quantizerFile.c_str(), m_pQuantizer.get());
    if (!m_pQuantizer)
    {
...
VectorIndex::LoadIndex(const std::string& p_loaderFilePath, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    if (iniReader.DoesSectionExist("Quantizer")) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Loading quantizer from LoadIndex metaStart: %d\n", metaStart);
        p_vectorIndex->SetQuantizer(SPTAG::COMMON::IQuantizer::LoadIQuantizer(handles[metaStart]));
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "LoadIndex loaded quantizer at %p\n", p_vectorIndex->m_pQuantizer.get());
...
VectorIndex::LoadIndexFromFile(const std::string& p_file, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    if (iniReader.DoesSectionExist("Quantizer"))
    {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Loading quantizer from LoadIndexFromDir\n");
        p_vectorIndex->SetQuantizer(SPTAG::COMMON::IQuantizer::LoadIQuantizer(fp));
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "LoadIndexFromDir loaded quantizer at %p\n", p_vectorIndex->m_pQuantizer.get());
        if (!p_vectorIndex->m_pQuantizer) return ErrorCode::FailedParseValue;
    }
...
VectorIndex::LoadIndex(const std::string& p_config, const std::vector<ByteArray>& p_indexBlobs, std::shared_ptr<VectorIndex>& p_vectorIndex)
{
    ...
    {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "Loading quantizer from LoadIndexFromMemory\n");
        p_vectorIndex->SetQuantizer(COMMON::IQuantizer::LoadIQuantizer(p_indexBlobs[4]));
        SPTAGLIB_LOG(Helper::LogLevel::LL_Error, "LoadIndexFromMemory loaded quantizer at %p\n", p_vectorIndex->m_pQuantizer.get());
        if (!p_vectorIndex->m_pQuantizer) return ErrorCode::FailedParseValue;
    }
...
// 这里需要量化器把输入的量化向量recontruct 成全精度向量俩比较久，因为index本身的数据是全精度向量
void VectorIndex::ApproximateRNG(std::shared_ptr<VectorSet>& fullVectors, std::unordered_set<SizeType>& exceptIDS, int candidateNum, Edge* selections, int replicaCount, int numThreads, int numTrees, int leafSize, float RNGFactor, int numGPUs)
{
        ...
        threads.emplace_back([&, tid]()
            {
                ...
                while (true)
                {
                    ...
                    if (m_pQuantizer)
                    {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "approximateRNG quantizer\n");
...
#define DefineVectorValueType(Name, Type) \
                    case VectorValueType::Name: \
                        (*((COMMON::QueryResultSet<Type>*)&resultSet)).SetTargetWithQuantized(reinterpret_cast<Type*>(reconstructed_vector), (const uint8_t*)fullVectors->GetVector(fullID), nullptr); \
                        break;
#include "inc/Core/DefinitionList.h"
#undef DefineVectorValueType
                    default:
