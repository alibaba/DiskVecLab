...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        {
            //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "refince centers\n");
            int maxcluster = -1;
            SizeType maxCount = 0;

// there should use quantizer l2_distance
            for (int k = 0; k < args._DK; k++) {
                if (args.counts[k] > maxCount && args.newCounts[k] > 0)
                {
                    float center_dist = 0;
                    if (args.m_pQuantizer) {
                        //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "refince centers mmmm\n");
                        center_dist = args.m_pQuantizer->L2Distance((const uint8_t*)data[args.clusterIdx[k]], (const uint8_t*)args.centers + k * args._D);
                    }
                    else {
                        //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "refince centers nnnn %d\n", args._D);
                        center_dist = DistanceUtils::ComputeDistance((T*)data[args.clusterIdx[k]], args.centers + k * args._D, args._D, DistCalcMethod::L2);
                    }
                    if (center_dist > 1e-6)
                    {
                        maxcluster = k;
            ...
            for (int k = 0; k < args._DK; k++) {
                ...
                if (args.counts[k] == 0) {
                    ...
                    if (args.m_pQuantizer) {
                        //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "refince centers here\n");
                ...
                if (args.m_pQuantizer) {
                    //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "refince centers there\n");
                    diff += args.m_pQuantizer->L2Distance((const uint8_t*)TCenter, (const uint8_t*)args.centers + k * args._D);
                }
                else {
...
#pragma omp parallel for num_threads(args._T) shared(data, indices) reduction(+:currDist)
        ...
        float TryClustering(const Dataset<T>& data,
            ...
            //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "TryClustering");
            ...
            for (int iter = 0; iter < 100; iter++) {
                ...
                */
                ...
                //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "refine centers");
...
#undef DefineVectorValueType
        ...
        public:
                ...
                bool dynamicK = false, IAbortOperation* abort = nullptr)
            {
                ...
                if (m_pQuantizer) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "build trees");
                } else {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "build trees no quantizer\n");
                }
