        ...
        public:
            ...
            static float CalculateRecall(VectorIndex* index, T* query, int K) {
                ...
                if (index->m_pQuantizer)
                {
                    ...
                    sampleANN.SetTargetWithQuantized(reconstructVector, (const uint8_t*)query, index->m_pQuantizer);
                    sampleTruth.SetTargetWithQuantized(reconstructVector,(const uint8_t*)query, index->m_pQuantizer);
                }
