...
namespace SPTAG
{
    namespace COMMON
    {
        ...
        void PQQuantizer<T>::QuantizeVector(const void* vec, std::uint8_t* vecout, bool ADC) const
        {
            if (ADC && GetEnableADC())
            {
                // Explicitly call global DistanceCalcSelector to avoid circular dependency
        ...
        void PQQuantizer<T>::InitializeDistanceTables()
        {
            ...
            // Explicitly call the global DistanceCalcSelector from COMMON namespace
            // to avoid circular dependency when T=uint8_t (member function would call this->L2Distance)
