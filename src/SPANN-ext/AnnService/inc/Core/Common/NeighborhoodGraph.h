...
#undef DefineVectorValueType
                ...
                std::vector<std::pair<SizeType, SizeType>>& leaves)
            {
                ...
                {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Debug, "TpTree Partitioning node [%d,%d]\n", first, last);
                    SizeType splitidx = first;
                    {
                        ...
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Debug, "TpTree Partitioning node [%d,%d] with %d samples and %d dimensions\n", first, last, count, cols);

                        std::vector<float> Mean(cols, 0);
                        // calculate the mean of each dimension
                        for (SizeType j = first; j <= end; j++)
                        {
							if (indices[j] > index->GetNumSamples()) {
								SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "error indices %d %d\n", j, indices[j]);
							}
                        ...
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Debug, "TpTree best mean: %f\n", bestmean);
                        ...
                        for (int i = 0; i < iIteration; i++)
                        {
                            ...
                            {
                                Val[j] = 0;
							if (indices[first + j] > index->GetNumSamples()) {
								SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "error indices %d %d\n", first + j, indices[first + j]);
							}
                        ...
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Debug, "TpTree best variance: %f\n", bestvariance);
                        SizeType i = first;
                        SizeType j = last;
                        ...
                        {
                            float val = 0;
							if (indices[i] > index->GetNumSamples()) {
								SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "error indices %d %d\n", i, indices[i]);
							}
                        ...
                        if ((i == first) || (i == last + 1))
                        {
                            i = (SizeType)(((int64_t)first + (int64_t)last + 1) / 2);
                        }
                        splitidx = i;
                    ...
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "TpTree split node [%d,%d] at %d into [%d,%d] and [%d,%d]\n", first, last, splitidx, first, splitidx - 1, splitidx, last);
...
#pragma omp parallel for schedule(dynamic)
                ...
                {
					SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "m_iTPTnumber %d\n", i);
                    Sleep(i * 100); std::srand(clock());
                    for (SizeType j = 0; j < m_iGraphSize; j++) TptreeDataIndices[i][j] = j;
					SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "iGraphSize %d\n", m_iGraphSize);

					for(int nnn = 0; nnn < TptreeDataIndices[i].size(); nnn++) {
						if (TptreeDataIndices[i][nnn] > m_iGraphSize) {
							SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "error TptreeDataIndices shouldn't larger than graphsize%d\n");
						}
					}
...
#pragma omp parallel for schedule(dynamic)
            ...
            void RefineNode(VectorIndex* index, const SizeType node, bool updateNeighbors, bool searchDeleted, int CEF)
            {
                ...
                if (index->m_pQuantizer) {
                    ...
                    //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Reconstruct Size %d\n", index->m_pQuantizer->ReconstructSize());
                    index->m_pQuantizer->ReconstructVector((const uint8_t*)query.GetTarget(), rec_query);
                    query.SetTargetWithQuantized((T*)rec_query, (const uint8_t*)query.GetTarget(), index->m_pQuantizer);
                }
