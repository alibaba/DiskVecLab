...
namespace SPTAG
{
    ...
    namespace BKT
    {
        ...
        private:
            ...
            std::function<float(const std::uint8_t*, const std::uint8_t*, DimensionType)> m_fComputeDistanceQuantizer;
            int m_iBaseSquare;
            
            std::shared_ptr<SPTAG::COMMON::IQuantizer> m_reQuantizer;
...
#undef DefineBKTParameter
                ...
                //m_fComputeDistanceQuantizer = std::function<float(const T*, const T*, DimensionType)>(COMMON::DistanceCalcSelector<T>(m_iDistCalcMethod));
            ...
            inline float ComputeDistance(const void* pX, const void* pY) const {
                //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "ComputeDistance\n");
            ...
            inline float ComputeDistanceQuantizer(const void* pX, const void* pY) const override {
                return m_fComputeDistanceQuantizer((const std::uint8_t*)pX, (const std::uint8_t*)pY, m_pSamples.C());
            }
        ...
        private:
            ...
            template <bool(*notDeleted)(const COMMON::Labelset&, SizeType), bool(*isDup)(COMMON::QueryResultSet<T>&, SizeType, float)>
