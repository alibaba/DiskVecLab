...
#undef DefineVectorValueType
                ...
                // 添加：打印所有选中的头向量ID
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "=== 打印所有选中的头向量ID ===\n");
                for (size_t i = 0; i < selected.size(); i++) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Head[%zu]: VectorID=%d\n", i, selected[i]);
                }
                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "=== 头向量ID打印完成 ===\n");
                ...
                if (!opts.m_noOutput)
                {
                    ...
                    // 添加：打印头向量的详细内容
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "=== 打印头向量详细内容 ===\n");
                    for (size_t i = 0; i < selected.size() && i < 10; i++) { // 只打印前10个，避免输出过多
                        uint64_t vid = static_cast<uint64_t>(selected[i]);
                        const void* vectorData = vectorSet->GetVector((SizeType)vid);

                        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Head[%zu] VectorID=%d, 前10维: ", i, selected[i]);



                        if (vectorSet->GetValueType() == VectorValueType::Float) {
                            const float* floatVec = static_cast<const float*>(vectorData);
                            for (int d = 0; d < std::min(10, (int)vectorSet->Dimension()); d++) {
                                SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "%.6f ", floatVec[d]);
                            }
                        }
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "\n");
                    }
                    if (selected.size() > 10) {
                        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "... 共%zu个头向量，仅显示前10个\n", selected.size());
                    }
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "=== 头向量内容打印完成 ===\n");
                }
