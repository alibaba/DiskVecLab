	...
	namespace SSDServing {
		namespace SSDIndex {
                ...
                for (int i = 0; i < p_numThreads; i++) { threads.emplace_back([&, i]()
                    {
                        ...
                        while (true)
                        {
                            ...
                            if (index < numQueries)
                            {
                                ...
                                p_index->GetMemoryIndex()->SearchIndex(p_results[index]); // 使用target
                                double endTime = threadws.getElapsedMs();
                                p_index->SearchDiskIndex(p_results[index], &(p_stats[index])); // 使用quantized target
            ...
            void Search(SPANN::Index<ValueType>* p_index)
            {
                ...
                {
                   SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "spannindex quantizer \n");
                ...
                if (!p_opts.m_vectorPath.empty() && fileexists(p_opts.m_vectorPath.c_str())) {
                    std::shared_ptr<Helper::ReaderOptions> vectorOptions(new Helper::ReaderOptions(p_opts.m_valueType, p_opts.m_dim, p_opts.m_fullVectorType, p_opts.m_vectorDelimiter));
                    //std::shared_ptr<Helper::ReaderOptions> vectorOptions(new Helper::ReaderOptions(VectorValueType::Float, 128, VectorFileType::XVEC, p_opts.m_vectorDelimiter));
                    //if (p_index->m_pQuantizer) {
                    //  vectorOptions->m_inputValueType = VectorValueType::UInt8;
                    //}
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "vector value origin type %d\n", p_opts.m_valueType);
                ...
                if (p_opts.m_rerank > 0 && vectorSet != nullptr) {
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "\n Begin rerank...\n");
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "result.size %d\n", results.size());
                    SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "internalResultNum %d\n", internalResultNum);
                    for (int i = 0; i < results.size(); i++)
                    {
                        for (int j = 0; j < internalResultNum; j++)
                        {
                            if (results[i].GetResult(j)->VID < 0) continue;
                            // Save the quantized distance before reranking
                            float quantized_dist = results[i].GetResult(j)->Dist;
                            // Compute real distance using full precision vectors
                            float real_dist = COMMON::DistanceUtils::ComputeDistance(
                                (const ValueType*)querySet->GetVector(i),
                                (const ValueType*)vectorSet->GetVector(results[i].GetResult(j)->VID), 
                                querySet->Dimension(), 
                                p_opts.m_distCalcMethod);
                            // Update with real distance
                                //SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "vid %d, real dist %f, quantized dist %f\n",
                                //results[i].GetResult(j)->VID, real_dist, quantized_dist);
                                results[i].GetResult(j)->Dist = real_dist;
                        }
                        BasicResult* re = results[i].GetResults();
                        std::sort(re, re + internalResultNum, COMMON::Compare);
                    }
                    //K = p_opts.m_rerank;
                }
