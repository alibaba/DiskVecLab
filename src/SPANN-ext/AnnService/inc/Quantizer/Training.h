...
public:
    QuantizerOptions(SizeType trainingSamples, bool debug, float lambda, SPTAG::QuantizerType qtype, std::string qfile, DimensionType qdim, std::string fullvecs, std::string recvecs) : Helper::ReaderOptions(VectorValueType::Float, 0, VectorFileType::TXT, "|", 32), m_trainingSamples(trainingSamples), m_debug(debug), m_KmeansLambda(lambda), m_quantizerType(qtype), m_outputQuantizerFile(qfile), m_quantizedDim(qdim), m_outputFullVecFile(fullvecs), m_outputReconstructVecFile(recvecs)
    {
        ...
        AddOptionalOption(m_outputReconstructVecFile, "-orv", "--output_reconstruct", "Output reconstructed vectors.");

        // RaBitQ specific options
        AddOptionalOption(m_rabitqBits, "-rbqbits", "--rabitq_bits", "Number of bits for RaBitQ quantization (default: 8).");
        
        // LSQ specific options
        AddOptionalOption(m_lsqBitsPerSubvector, "-lsqbits", "--lsq_bits", "Number of bits per subvector for LSQ quantization (default: 8).");
        AddOptionalOption(m_lsqNumSubvector, "-lsq_subvector", "--lsq_subvector", "Number of subvector for LSQ quantization (default: 4).");
    }
    ...
    // RaBitQ specific options
    SizeType m_rabitqBits = 8;
    
    // LSQ specific options
    SizeType m_lsqBitsPerSubvector = 8;

    SizeType m_lsqNumSubvector = 4;
};
...
#pragma omp parallel for
    for (int codebookIdx = 0; codebookIdx < options->m_quantizedDim; codebookIdx++) {
        SPTAGLIB_LOG(Helper::LogLevel::LL_Info, "Training Codebook %d.\n", codebookIdx);        auto kargs = COMMON::KmeansArgs<T>(numCentroids, subdim, raw_vectors->Count(), options->m_threadNum, DistCalcMethod::L2, nullptr);
...
// num_points is the number of points to quantizer
template <typename T>
std::unique_ptr<COMMON::IQuantizer> TrainRaBitQQuantizer(std::shared_ptr<QuantizerOptions> options, std::shared_ptr<VectorSet> raw_vectors, int64_t num_points)
{
    auto quantizer = std::make_unique<COMMON::RaBitQQuantizer>(raw_vectors->Dimension(), options->m_rabitqBits, num_points);
    
    // Convert VectorSet to float array
    std::unique_ptr<float[]> float_data = std::make_unique<float[]>(raw_vectors->Count() * raw_vectors->Dimension());
    
    for (SizeType i = 0; i < raw_vectors->Count(); i++) {
        auto vec = reinterpret_cast<const T*>(raw_vectors->GetVector(i));
        // Convert to float
        for (DimensionType j = 0; j < raw_vectors->Dimension(); j++) {
            float_data[i * raw_vectors->Dimension() + j] = static_cast<float>(vec[j]);
        }
    }
    
    // Train the quantizer with float data
    quantizer->Train(raw_vectors->Count(), float_data.get());
    
    return quantizer;
}

template <typename T>
std::unique_ptr<COMMON::IQuantizer> TrainLSQQuantizer(std::shared_ptr<QuantizerOptions> options, std::shared_ptr<VectorSet> raw_vectors)
{
    auto quantizer = std::make_unique<COMMON::LSQQuantizer>(raw_vectors->Dimension(), options->m_lsqNumSubvector, options->m_lsqBitsPerSubvector);
