...
static inline void flip_sign(const uint8_t* flip, float* data, size_t dim) {
    ...
    constexpr size_t kFloatsPerChunk = 32;  // Process 32 floats per iteration for AVX2
    const __m256 sign_flip = _mm256_castsi256_ps(_mm256_set1_epi32(0x80000000));
    
    for (size_t i = 0; i < dim; i += kFloatsPerChunk) {
        // Load 32 bits (4 bytes) from the bit sequence for AVX2
        uint32_t mask_bits;
        std::memcpy(&mask_bits, &flip[i / 8], sizeof(mask_bits));
        
        // Process 8 floats at a time (4 iterations for 32 floats)
        for (size_t j = 0; j < 4; ++j) {
            __m256 vec = _mm256_loadu_ps(&data[i + j * 8]);
            uint8_t mask = static_cast<uint8_t>((mask_bits >> (j * 8)) & 0xFF);
            
            // Create mask for each float in the 8-float vector
            __m256 flip_mask = _mm256_setzero_ps();
            for (int k = 0; k < 8; ++k) {
                if (mask & (1 << k)) {
                    reinterpret_cast<uint32_t*>(&flip_mask)[k] = 0xFFFFFFFF;
                }
            }
            
            vec = _mm256_xor_ps(vec, _mm256_and_ps(flip_mask, sign_flip));
            _mm256_storeu_ps(&data[i + j * 8], vec);
        }
    }
#endif
}
