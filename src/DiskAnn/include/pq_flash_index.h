...
template <typename T, typename LabelT = uint32_t> class PQFlashIndex
{
  public:
    ...
    DISKANN_DLLEXPORT int load(uint32_t num_threads, const char *index_prefix,
                               PQType pq_type = PQType::PQ, std::string dat_path = "");
#endif
    ...
    DISKANN_DLLEXPORT int load_from_separate_paths(diskann::MemoryMappedFiles &files, uint32_t num_threads,
                                                   ...
                                                   PQType pq_type = PQType::PQ, std::string data_path = "");
#else
    ...
    // ==================== Resumable Search APIs ====================
    // These APIs support pause/resume functionality for iterative search

    /**
     * Initialize a resumable search with the given parameters.
     * This sets up the search state and performs the initial search up to l_search.
     * 
     * @param query Query vector
     * @param k_search Number of results to return
     * @param l_search Initial search list size (L parameter)
     * @param beam_width Beam width for IO batching
     * @param state Output search state (will be initialized)
     * @param io_limit Maximum number of IO operations (default: unlimited)
     * @param stats Optional statistics output
     */
    DISKANN_DLLEXPORT void init_search(const T *query, const uint64_t k_search, const uint64_t l_search,
                                       const uint64_t beam_width, SearchState<T> &state,
                                       const uint32_t io_limit = std::numeric_limits<uint32_t>::max(),
                                       QueryStats *stats = nullptr);

    /**
     * Resume a previously paused search with an expanded L value.
     * This continues the search from the saved state without re-visiting
     * already expanded nodes.
     * 
     * @param query Query vector (must be the same as init_search)
     * @param k_search Number of results to return
     * @param new_l_search New (expanded) search list size
     * @param beam_width Beam width for IO batching
     * @param state Search state from init_search or previous resume_search
     * @param io_limit Maximum additional IO operations (default: unlimited)
     * @param stats Optional statistics output (accumulates with previous stats)
     */
    DISKANN_DLLEXPORT void resume_search(const T *query, const uint64_t k_search, const uint64_t new_l_search,
                                         const uint64_t beam_width, SearchState<T> &state,
                                         const uint32_t io_limit = std::numeric_limits<uint32_t>::max(),
                                         QueryStats *stats = nullptr);
     ...
     * Can be called after init_search or resume_search.
     * 
     * @param k_search Number of results to return
     * @param state Search state
     * @param res_ids Output array for result IDs
     * @param res_dists Output array for result distances
     * @param use_reorder_data Whether to use full precision reordering
     */
    DISKANN_DLLEXPORT void get_search_results(const uint64_t k_search, const SearchState<T> &state,
    ...
    DISKANN_DLLEXPORT void set_rabitq_params(bool use_fastscan, bool use_hacc);
