...
#pragma omp parallel for schedule(dynamic, 1) num_threads(nthreads)
        ...
        double lat_999 = diskann::get_percentile_stats<float>(
            stats, tuning_sample_num, 0.999f, [](const diskann::QueryStats &stats) { return stats.total_us.load(); });

        double mean_latency = diskann::get_mean_stats<float>(
            stats, tuning_sample_num, [](const diskann::QueryStats &stats) { return stats.total_us.load(); });
...
int build_disk_index(const char *dataFilePath, const char *indexFilePath, const char *indexBuildParameters,
                     ...
                     const uint32_t Lf, const size_t rabitq_clusters)
{
    ...
    std::string faiss_lsq_index_path = index_prefix_path + "_faiss_lsq.index";
    std::string faiss_rabitq_index_path = index_prefix_path + "_faiss_rabitq.index";
    ...
    std::string disk_rabitq_index_path = index_prefix_path + "_faiss_rabitq.index";
    std::string disk_lsq_index_path = index_prefix_path + "_faiss_lsq.index";
    ...
    double final_index_ram_limit = atof(param_list[2].c_str());// get_memory_budget(param_list[2]);
    ...
    size_t num_pq_chunks = static_cast<size_t>(final_index_ram_limit);
    std::cout << "Number of PQ bytes initially derived from search_DRAM_budget (B): " << num_pq_chunks
              << std::endl;
    ...
    generate_quantized_data_build_disk_index<T>(data_file_to_use, pq_pivots_path, pq_compressed_vectors_path, faiss_lsq_index_path,
                               faiss_rabitq_index_path, compareMetric, p_val,
                               num_pq_chunks, use_opq, codebook_prefix, rabitq_clusters);
