...
static inline void log_timed_block(const char *label, Fn &&fn)
{
    diskann::cout << "[TIMER] " << label << " start @ " << current_time_string() << std::endl;
    const auto t0 = std::chrono::steady_clock::now();
    fn();
    const auto t1 = std::chrono::steady_clock::now();
    const auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
    diskann::cout << "[TIMER] " << label << " end   @ " << current_time_string() << " (elapsed " << ms
                 << " ms, " << (static_cast<double>(ms) / 1000.0) << " s)" << std::endl;
}
...
int generate_rabitq_data(const float *train_data,const std::string data_file_path,
                         const std::string &index_file_path, int64_t dim, int64_t num_points, int64_t num_pq_chunks,
                         const std::string& data_type, size_t rabitq_clusters) {
    auto pq_table = PQFactory::create_pq_table(PQType::RABITQ, dim, num_pq_chunks, rabitq_clusters);
    pq_table->train(num_points, train_data, data_file_path, index_file_path, data_type);
    return 0;
}

int generate_lsq_data(const float *train_data, const std::string data_file_path,
                      const std::string &index_file_path,
                      int64_t dim, int64_t num_points, int64_t num_pq_chunks,
                      const std::string& data_type) {
    auto pq_table = PQFactory::create_pq_table(PQType::LSQ, dim, num_pq_chunks);
    pq_table->train(num_points, train_data, data_file_path, index_file_path, data_type);
    return 0;
}
...
template <typename T>
void generate_quantized_data_build_disk_index(const std::string &data_file_to_use, const std::string &pq_pivots_path,
                             const std::string &pq_compressed_vectors_path,
                             const std::string &disk_lsq_index_path,
                             const std::string &disk_rabitq_index_path,
                             ...
                             const size_t rabitq_clusters)
{
    // Determine data type string from template parameter
    ...
    if (!file_exists(codebook_prefix))
    {
        ...
        switch (pq_type) {
        case PQType::PQ:
            log_timed_block("PQ: generate pivots", [&]() {
            ...
            log_timed_block("PQ: generate quantized codes", [&]() {
        ...
        case PQType::RABITQ:
            if (num_pq_chunks > 8) {
                diskann::cout << "Training RabitQ index with " << rabitq_clusters << " clusters" << std::endl;
                log_timed_block("RabitQ: generate quantized codes", [&]() {
                    generate_rabitq_data(train_data, data_file_to_use, disk_rabitq_index_path, (uint32_t)train_dim,
                                        train_size, num_pq_chunks, data_type_str, rabitq_clusters);
                });
            }
            [[fallthrough]];
        case PQType::LSQ:
            if (num_pq_chunks <= 64) {
                diskann::cout << "Training LSQ index" << std::endl;
                log_timed_block("LSQ: generate quantized codes", [&]() {
                    generate_lsq_data(train_data, data_file_to_use, disk_lsq_index_path, (uint32_t)train_dim,
                                      train_size, num_pq_chunks, data_type_str);
                });
            } else {
                diskann::cout << "Skipping LSQ training for num_pq_chunks >= 64" << std::endl;
            }
            delete[] train_data;
            diskann::cout << "TEMP: ONLY TRAIN INDEX, EXIT NOW" << std::endl;
            exit(EXIT_SUCCESS);
            break;
        case PQType::OPQ:
                             ...
                             const std::string &codebook_prefix)
{
    ...
    if (!file_exists(codebook_prefix))
    {
        ...
        switch (pq_type) {
            ...
            case PQType::RABITQ:
                log_timed_block("RabitQ: generate quantized codes", [&]() {
                    generate_rabitq_data(train_data, "", "", (uint32_t)train_dim, train_size, num_pq_chunks, "float",
                                         RABITQ_DEFAULT_NUM_CLUSTERS);
                });
                delete[] train_data;
                break;
            case PQType::LSQ:
                log_timed_block("LSQ: generate quantized codes", [&]() {
                    generate_lsq_data(train_data, "", "", (uint32_t)train_dim, train_size, num_pq_chunks, "float");
                });
                delete[] train_data;
                break;
            case PQType::OPQ:
                log_timed_block("OPQ: generate pivots", [&]() {
                ...
                delete[] train_data;
                log_timed_block("OPQ: generate quantized codes", [&]() {
                    generate_pq_data_from_pivots<T>(data_file_to_use, NUM_PQ_CENTROIDS, (uint32_t)num_pq_chunks,
...
template DISKANN_DLLEXPORT void generate_quantized_data_build_disk_index<int8_t>(const std::string &data_file_to_use,
                                                                const std::string &pq_pivots_path,
                                                                const std::string &pq_compressed_vectors_path,
                                                                const std::string &disk_lsq_index_path,
                                                                const std::string &disk_rabitq_index_path,
...
template DISKANN_DLLEXPORT void generate_quantized_data_build_disk_index<uint8_t>(const std::string &data_file_to_use,
                                                                 const std::string &pq_pivots_path,
                                                                 const std::string &pq_compressed_vectors_path,
                                                                 const std::string &disk_lsq_index_path,
                                                                 const std::string &disk_rabitq_index_path,
...
template DISKANN_DLLEXPORT void generate_quantized_data_build_disk_index<float>(const std::string &data_file_to_use,
                                                               const std::string &pq_pivots_path,
                                                               const std::string &pq_compressed_vectors_path,
                                                               const std::string &disk_lsq_index_path,
                                                               const std::string &disk_rabitq_index_path,
