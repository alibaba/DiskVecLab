...
// Helper function to convert template type T to string
template <typename T>
inline std::string get_data_type_string() {
...
#ifdef EXEC_ENV_OLS
template <typename T, typename LabelT>
int PQFlashIndex<T, LabelT>::load(MemoryMappedFiles &files, uint32_t num_threads, const char *index_prefix, PQType pq_type, std::string data_path)
{
#else
template <typename T, typename LabelT> int PQFlashIndex<T, LabelT>::load(uint32_t num_threads, const char *index_prefix, PQType pq_type, std::string data_path)
{
#endif
    ...
    std::string faiss_index_file = std::string(index_prefix) + "_faiss_rabitq.index";
    if (pq_type == PQType::LSQ) {
        faiss_index_file = std::string(index_prefix) + "_faiss_lsq.index";
    } else if (pq_type == PQType::RABITQ) {
        faiss_index_file = std::string(index_prefix) + "_faiss_rabitq.index";
    }
    _pq_table = PQFactory::create_pq_table(pq_type);
#ifdef EXEC_ENV_OLS
    return load_from_separate_paths(files, num_threads, _disk_index_file.c_str(), pq_table_bin.c_str(),
                                    pq_compressed_vectors.c_str(), faiss_index_file.c_str(), pq_type, data_path);
#else
    return load_from_separate_paths(num_threads, _disk_index_file.c_str(), pq_table_bin.c_str(),
                                    pq_compressed_vectors.c_str(), faiss_index_file, pq_type, data_path);
#endif
}
...
int PQFlashIndex<T, LabelT>::load_from_separate_paths(diskann::MemoryMappedFiles &files, uint32_t num_threads,
                                                      ...
                                                      const char *compressed_filepath, std::string faiss_index_filepath,
                                                      PQType pq_type, std::string data_path)
{
#else
template <typename T, typename LabelT>
int PQFlashIndex<T, LabelT>::load_from_separate_paths(uint32_t num_threads, const char *index_filepath,
                                                      const char *pivots_filepath, const char *compressed_filepath,
                                                      std::string faiss_index_filepath,
                                                      PQType pq_type, std::string data_path)
{
#endif
    ...
    if (pq_type == PQType::RABITQ || pq_type == PQType::LSQ)
    {
        // do nothing for now
    } else {
#ifdef EXEC_ENV_OLS
    ...
    {
        diskann::cout << "Error. Number of PQ centroids is not 256. Exiting." << pq_file_num_centroids << std::endl;
        return -1;
    }

    size_t npts_u64, nchunks_u64;
    std::string data_type_str = get_data_type_string<T>();
    
    if (pq_type == PQType::RABITQ || pq_type == PQType::LSQ)
    {
        _pq_table->load_pq_compressed_vectors(faiss_index_filepath, this->data, data_type_str);
        this->_num_points = _pq_table->get_num_points();
    } else if (pq_type == PQType::FULL_PRECISION) {
        // full precision load origin data directly
        _pq_table->load_pq_compressed_vectors(data_path, this->data, data_type_str);
        this->_num_points = _pq_table->get_num_points();
    } else {
#ifdef EXEC_ENV_OLS
    ...
    diskann:cout << "num_point ,dim, disk_bytes_per_point, _aligned_dim, nchunks_u64 " << _num_points << " "
             << _data_dim << " " << _disk_bytes_per_point << " " << _aligned_dim << " " << nchunks_u64 << std::endl;
                                                 ...
                                                 QueryStats *stats)
{
    ...
    std::unique_ptr<DistanceComputer> dc_computer(_pq_table->preprocess_query(query_rotated)); // center the query and rotate if
...
uint8_t *data, uint8_t *pq_coord_scratch, float* pq_dists) override {
                 */
                _pq_table->compute_dists(&medoid_ids[cur_m], 1, dist_scratch, this->data, pq_coord_scratch, pq_dists, dc_computer.get());
    ...
    _pq_table->compute_dists(&best_medoid, 1, dist_scratch, this->data, pq_coord_scratch, pq_dists, dc_computer.get());
    ...
    while (retset.has_unexpanded_node() && num_ios < io_limit)
    {
        ...
        for (auto &cached_nhood : cached_nhoods)
        {
            ...
            _pq_table->compute_dists(node_nbrs, nnbrs, dist_scratch, this->data, pq_coord_scratch, pq_dists, dc_computer.get());
            if (stats != nullptr)
            {
...
// ==================== Resumable Search Implementation ====================

template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::init_search(const T *query1, const uint64_t k_search, const uint64_t l_search,
                                          const uint64_t beam_width, SearchState<T> &state,
                                          const uint32_t io_limit, QueryStats *stats)
{
    // Validate parameters
    uint64_t num_sector_per_nodes = DIV_ROUND_UP(_max_node_len, defaults::SECTOR_LEN);
    if (beam_width > num_sector_per_nodes * defaults::MAX_N_SECTOR_READS)
    ...
    // Copy query to thread specific aligned memory
    float query_norm = 0;
    T *aligned_query_T = query_scratch->aligned_query_T();
    float *query_float = pq_query_scratch->aligned_query_float;
    float *query_rotated = pq_query_scratch->rotated_query;
    ...
    std::unique_ptr<DistanceComputer> dc_computer(_pq_table->preprocess_query(query_rotated));
    float *pq_dists = pq_query_scratch->aligned_pqtable_dist_scratch;
    ...
    // Use state's visited and retset
    auto &visited = state.visited();
    auto &retset = state.retset();
    ...
    // Insert medoid into retset and mark as visited
    _pq_table->compute_dists(&best_medoid, 1, dist_scratch, this->data, pq_coord_scratch, pq_dists, dc_computer.get());
    retset.insert(Neighbor(best_medoid, dist_scratch[0]));
    visited.insert(best_medoid);
        ...
        for (auto &cached_nhood : cached_nhoods)
        {
            ...
            for (uint64_t m = 0; m < nnbrs; ++m)
            {
                ...
                if (visited.insert(id).second)
                {
                    ...
                    Neighbor nn(id, dist);
                    retset.insert(nn);
                    // Track ALL discovered candidates for capacity expansion
                    state.add_candidate(nn);
                }
            }
        }
...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::resume_search(const T *query1, const uint64_t k_search, const uint64_t new_l_search,
                                            const uint64_t beam_width, SearchState<T> &state,
                                            const uint32_t io_limit, QueryStats *stats)
{
    if (!state.is_initialized())
    {
        throw ANNException("SearchState not initialized. Call init_search first.", -1, __FUNCSIG__, __FILE__, __LINE__);
    }

    // Expand capacity if needed
    if (new_l_search > state.get_current_L())
    {
        state.expand_capacity(new_l_search);
    }

    // If no more unexpanded nodes, nothing to do
    if (!state.has_unexpanded_node())
    {
        return;
    }
    ...
    uint32_t num_ios = 0;
    uint32_t additional_io_limit = io_limit;
    ...
    // Continue beam search from where we left off
    while (retset.has_unexpanded_node() && num_ios < additional_io_limit)
    {
        frontier.clear();
        frontier_nhoods.clear();
        frontier_read_reqs.clear();
...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::get_search_results(const uint64_t k_search, const SearchState<T> &state,
                                                 uint64_t *res_ids, float *res_dists,
                                                 const bool use_reorder_data)
{
    // Sort full_retset by distance
    auto full_retset_copy = state.full_retset();
    std::sort(full_retset_copy.begin(), full_retset_copy.end());
    ...
    if (use_reorder_data)
    {
        ...
        {
            throw ANNException("Requested use of reordering data which does "
                               "not exist in index file",
                               -1, __FUNCSIG__, __FILE__, __LINE__);
        }

        // Get thread-local scratch space for reordering
        ScratchStoreManager<SSDThreadData<T>> manager(this->_thread_data);
        auto data = manager.scratch_space();
        IOContext &ctx = data->ctx;
        auto query_scratch = &(data->scratch);
        ...
        if (full_retset_copy.size() > k_search * FULL_PRECISION_REORDER_MULTIPLIER)
            full_retset_copy.erase(full_retset_copy.begin() + k_search * FULL_PRECISION_REORDER_MULTIPLIER, full_retset_copy.end());

        for (size_t i = 0; i < full_retset_copy.size(); ++i)
        {
            vec_read_reqs.emplace_back(VECTOR_SECTOR_NO(((size_t)full_retset_copy[i].id)) * defaults::SECTOR_LEN,
                                       defaults::SECTOR_LEN, sector_scratch + i * defaults::SECTOR_LEN);
        }
        ...
        for (size_t i = 0; i < full_retset_copy.size(); ++i)
        {
            auto id = full_retset_copy[i].id;
            auto location = (sector_scratch + i * defaults::SECTOR_LEN) + VECTOR_SECTOR_OFFSET(id);
            full_retset_copy[i].distance = _dist_cmp->compare(aligned_query_T, (T *)location, (uint32_t)this->_data_dim);
        }
    ...
    // Copy k_search values
    size_t result_count = std::min(k_search, (uint64_t)full_retset_copy.size());
    for (uint64_t i = 0; i < result_count; i++)
    {
        res_ids[i] = full_retset_copy[i].id;
        auto key = (uint32_t)res_ids[i];
        if (_dummy_pts.find(key) != _dummy_pts.end())
        {
            res_ids[i] = _dummy_to_real_map[key];
        }

        if (res_dists != nullptr)
        {
            res_dists[i] = full_retset_copy[i].distance;
        }
    }
}
...
template <typename T, typename LabelT>
void PQFlashIndex<T, LabelT>::set_rabitq_params(bool use_fastscan, bool use_hacc)
{
    // If we're using a RabitqQuantizer, set its parameters
    if (auto* rabitq_table = dynamic_cast<RabitqQuantizer*>(_pq_table.get())) {
        rabitq_table->set_rabitq_params(use_fastscan, use_hacc);
        diskann::cout << "Setting RabitQ parameters: use_fastscan=" << use_fastscan 
                      << ", use_hacc=" << use_hacc << std::endl;
    }
}
