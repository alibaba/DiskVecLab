...
int search_disk_index(diskann::Metric &metric, const std::string &index_path_prefix,
                      ...
                      const std::vector<std::string> &query_filters, std::string data_path,
                      const bool use_reorder_data = false, std::string quantification_type = "PQ",
                      const bool use_rabitq_fastscan = true, const bool rabit_use_hacc = true)
{
    ...
    int res = _pFlashIndex->load(num_threads, index_path_prefix.c_str(), pq_type, data_path);

    // Set RabitQ parameters if using RabitQ quantization
    if (pq_type == PQType::RABITQ) {
        _pFlashIndex->set_rabitq_params(use_rabitq_fastscan, rabit_use_hacc);
    }
...
#pragma omp parallel for schedule(dynamic, 1)
    ...
    diskann::cout << std::setw(6) << "L" << std::setw(12) << "Beamwidth" << std::setw(16) << "QPS" << std::setw(16)
                  ...
                  << "Mean IO (us)" << std::setw(16) << "CPU (us)" << std::setw(16) << "Mean IOLat" << std::setw(16) << "IOLat Ratio" << std::setw(16) << "Mean DCs";
    if (calc_recall_flag)
    {
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        auto mean_latency = diskann::get_mean_stats<uint64_t>(
            stats, query_num, [](const diskann::QueryStats &stats) { return stats.total_us.load(); });

        auto latency_999 = diskann::get_percentile_stats<uint64_t>(
            stats, query_num, 0.999, [](const diskann::QueryStats &stats) { return stats.total_us.load(); });

        auto mean_ios = diskann::get_mean_stats<uint32_t>(stats, query_num,
                                                          [](const diskann::QueryStats &stats) { return stats.n_ios.load(); });

        auto mean_cpuus = diskann::get_mean_stats<uint64_t>(stats, query_num,
                                                         [](const diskann::QueryStats &stats) { return stats.cpu_us.load(); });

        auto mean_io_us = diskann::get_mean_stats<uint64_t>(stats, query_num,
                                                         [](const diskann::QueryStats &stats) { return stats.io_us.load(); });

        auto mean_cmps = diskann::get_mean_stats<uint64_t>(stats, query_num,
                                                          [](const diskann::QueryStats &stats) { return stats.n_cmps.load(); });

        auto mean_io_lat = mean_io_us / (double)mean_ios;
        auto io_lat_ratio = mean_io_us / (double)mean_latency;
        ...
        diskann::cout << std::setw(6) << L << std::setw(12) << optimized_beamwidth << std::setw(16) << qps
                      ...
                      << std::setw(16) << mean_io_us << std::setw(16) << mean_cpuus << std::setw(16) << mean_io_lat << std::setw(16) << io_lat_ratio << std::setw(16) << mean_cmps;
        if (calc_recall_flag)
        {
...
int main(int argc, char **argv)
{
    std::string data_type, dist_fn, index_path_prefix, result_path_prefix, query_file, gt_file, filter_label,
        label_type, query_filters_file, quantification_type, data_path;
    ...
    try
    {
        ...
        required_configs.add_options()("quantification_type,Q", po::value<std::string>(&quantification_type)->required(),
        ...
        optional_configs.add_options()("data_path",
    ...
    if (quantification_type == "FLAT" && data_path.empty())
    {
        std::cout << "Error: data_path is required for full precision quantization" << std::endl;
        return -1;
    }
        ...
        if (!query_filters.empty() && label_type == "ushort")
        {
            if (data_type == std::string("float"))
                return search_disk_index<float, uint16_t>(
                    ...
                    num_nodes_to_cache, search_io_limit, Lvec, fail_if_recall_below, query_filters, data_path,
                    use_reorder_data, quantification_type, use_rabitq_fastscan, rabit_use_hacc);
        ...
        else
        {
            if (data_type == std::string("float"))
                return search_disk_index<float>(metric, index_path_prefix, result_path_prefix, query_file, gt_file,
                                                ...
                                                fail_if_recall_below, query_filters, data_path, use_reorder_data, quantification_type, use_rabitq_fastscan, rabit_use_hacc);
            else if (data_type == std::string("int8"))
