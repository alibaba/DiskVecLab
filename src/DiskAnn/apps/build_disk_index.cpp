...
int main(int argc, char **argv)
{
    std::string data_type, dist_fn, data_path, index_path_prefix, codebook_prefix, label_file, universal_label,
        label_type;
    uint32_t num_threads, R, L, disk_PQ, build_PQ, QD, Lf, filter_threshold, rabitq_clusters;
    float B, M, alpha;
    ...
    try
    {
        ...
        optional_configs.add_options()("alpha", po::value<float>(&alpha)->default_value(2.0f),
        ...
        optional_configs.add_options()("rabitq_clusters",
                                       po::value<uint32_t>(&rabitq_clusters)->default_value(0),
                                       "Number of clusters for RaBitQ multi-cluster quantization. "
                                       "0 means use default (16). Higher values may improve recall "
        ...
        if (label_file != "" && label_type == "ushort")
        {
            if (data_type == std::string("int8"))
                return diskann::build_disk_index<int8_t>(data_path.c_str(), index_path_prefix.c_str(), params.c_str(),
                                                         ...
                                                         universal_label, filter_threshold, Lf, rabitq_clusters);
                ...
                return diskann::build_disk_index<uint8_t, uint16_t>(
                    ...
                    use_filters, label_file, universal_label, filter_threshold, Lf, rabitq_clusters);
