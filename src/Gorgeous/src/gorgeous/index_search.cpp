  ...
  void DecoIndex<T>::page_search(
    ...
    pool->runTask([&, this](int tid) {
      ...
      while(true) {
        ...
        if (io_limit <= 1) {
          _u64 t = 0;
          for (_u64 i = 0; i < cur_list_size && t < k_search; i++) {
            indices[t] = retset[i].id;
            if (distances != nullptr) {
              distances[t] = retset[i].distance;
              if (metric == diskann::Metric::INNER_PRODUCT) {
          ...
          if (t < k_search) {
            const _u64 fallback_id = (t > 0) ? indices[t - 1] : 0;
            const float fallback_dist = std::numeric_limits<float>::max();
            while (t < k_search) {
              indices[t] = fallback_id;
              if (distances != nullptr) {
                distances[t] = fallback_dist;
              }
              t++;
            }
          }
        ...
        while (num_ios < io_limit) {
          ...
          if (n_proc_in_q > 0) {
            ...
            auto it_fn = sec_buf2ftr.find(sector_buf);
            if (it_fn == sec_buf2ftr.end() || it_fn->second == nullptr) {
            ...
            for (unsigned j = 0; j < p_size; ++j) {
              ...
              vis_cand.emplace_back(id, node_buf);
              cand_size++;
            }
          ...
          if (n_io_in_q > 0) {
            ...
            for (int i = n_read_blks - 1; i >= 0; i--) {
              auto it = sec_buf2ftr.find(tmp_bufs[i]);
              if (it == sec_buf2ftr.end() || it->second == nullptr) {
                continue;
              }
            ...
            while (marker < cur_list_size && num_seen < beam_width) {
              if (retset[marker].flag) {
                ...
                if (mem_pos != INF) {
                  ...
                  if (retset[marker].id >= id2page_.size()) {
                    marker++;
                    continue;
                  }
                  auto pid = id2page_[retset[marker].id];
                  if (pid >= gp_layout_.size()) {
                    marker++;
                    continue;
                  }
        ...
        if (embedding_search_L > cur_list_size) embedding_search_L = cur_list_size;
        while (l_idx < embedding_search_L) {
          ...
          for (_u32 ord_idx = l_idx; l_idx - ord_idx < MAX_N_SECTOR_READS && l_idx < embedding_search_L; l_idx++) {
            ...
            if (retset[l_idx].id >= id2page_.size()) {
              continue;
            }
          ...
          while (n_ops > 0) {
            ...
            for (int i = 0; i < n_read_blks; i++) {
              ...
              auto it_pid = sec_buf2pid.find(sector_buf);
              if (it_pid == sec_buf2pid.end()) {
                continue;
              }
              auto pid = it_pid->second;
