...
echo "Dataset: ${PREFIX}, R=${R}, L=${BUILD_L}, M=${M}, N_PQ_CODE=${N_PQ_CODE}"
echo "Disk Sector Length: ${SECTOR_LEN}, Graph Replicated Layout Sector Length: ${GR_SECTOR_LEN}"
echo "K: ${K}, Data Type: ${DATA_TYPE}, Distance Function: ${DIST_FN}"
echo "Base Path: ${BASE_PATH}"
echo "Query File: ${QUERY_FILE}"
echo "GT File: ${GT_FILE}"
echo "Build Threads: ${BUILD_T}"
echo "Graph Partition Times: ${GP_TIMES}, Graph Partition Threads: ${GP_T}, GP Lock Nums: ${GP_LOCK_NUMS}, GP Cut: ${GP_CUT}"
echo "DATA_DIR: ${DATA_DIR}"

INDEX_PREFIX_PATH="${PREFIX}_M${M}_R${R}_L${BUILD_L}/"
PQ_PREFIX_PATH="${INDEX_PREFIX_PATH}/PQ/C/"
RELATIVE_PQ_PREFIX_PATH="PQ/C/"
if [ ! $N_PQ_CODE -eq 0 ]; then
  PQ_PREFIX_PATH="${INDEX_PREFIX_PATH}/PQ/C${N_PQ_CODE}/"
  RELATIVE_PQ_PREFIX_PATH="PQ/C${N_PQ_CODE}/"
else
  echo "N_PQ_CODE should not be zero."
  exit 1
fi

MEM_SAMPLE_PATH="${INDEX_PREFIX_PATH}/MEM_SAMPLE/SAMPLE_RATE_${MEM_RAND_SAMPLING_RATE}/"
RELATIVE_MEM_SAMPLE_PATH="MEM_SAMPLE/SAMPLE_RATE_${MEM_RAND_SAMPLING_RATE}/"
MEM_INDEX_PATH="${INDEX_PREFIX_PATH}/MEM_INDEX/MEM_R_${MEM_R}_L_${MEM_BUILD_L}_ALPHA_${MEM_ALPHA}_RANDOM_RATE${MEM_RAND_SAMPLING_RATE}/"
RELATIVE_MEM_INDEX_PATH="MEM_INDEX/MEM_R_${MEM_R}_L_${MEM_BUILD_L}_ALPHA_${MEM_ALPHA}_RANDOM_RATE${MEM_RAND_SAMPLING_RATE}/"
GP_PATH="${INDEX_PREFIX_PATH}GP_TIMES_${GP_TIMES}_LOCK_${GP_LOCK_NUMS}_CUT${GP_CUT}/"

GRAPH_PATH="${INDEX_PREFIX_PATH}GRAPH/"
RELATIVE_GRAPH_PATH="GRAPH/"
GRAPH_REP_INDEX_PATH="${INDEX_PREFIX_PATH}GRAPH_CACHE_INDEX/"
RELATIVE_GRAPH_REP_INDEX_PATH="GRAPH_CACHE_INDEX/"
...
print_usage_and_exit() {
  echo "Usage: ./run_benchmark_multi_split.sh [debug/release] [build/build_pq/build_mem/gp/gr_layout/split_graph/search/search_split] [knn/range]"
  exit 1
}
...
echo "Parent Index Path: ${PARENT_INDEX_PATH}"
date
case $2 in
  build)
  ...
  gp)
    ...
    if [ -d ${GP_PATH} ]; then
      ...
      if [ -f "${GP_PATH}_part_tmp.index" ]; then
        ...
        ln -sf ${PARENT_INDEX_PATH}/${GP_PATH}_part_tmp.index ${INDEX_PREFIX_PATH}_disk.index
        ln -s ${PARENT_INDEX_PATH}/${GP_FILE_PATH} ${INDEX_PREFIX_PATH}_partition.bin
        exit 0
      else
  ...
  gr_layout)
    ...
    if [ -d ${GRAPH_CACHE_INDEX_GP_PATH} ]; then
      ...
      if [ -f "${GRAPH_CACHE_INDEX_GP_PATH}_part_tmp.index" ]; then
        ...
        ln -s ${PARENT_INDEX_PATH}/${GRAPH_CACHE_INDEX_GP_PATH}_part_tmp.index ${GRAPH_REP_INDEX_PATH}_graph_rep.index
        ln -s ${PARENT_INDEX_PATH}/${GP_FILE_PATH} ${GRAPH_REP_INDEX_PATH}_partition.bin
        exit 0
      else
  ...
  split_graph)
    ...
    if [ -d ${GRAPH_GP_PATH} ]; then
      ...
      if [ -f "${GRAPH_GP_PATH}_part_tmp.index" ]; then
        ...
        ln -s ${PARENT_INDEX_PATH}/${GRAPH_GP_PATH}_part_tmp.index ${GRAPH_PATH}_disk_graph.index
        ln -s ${PARENT_INDEX_PATH}/${GP_FILE_PATH} ${GRAPH_PATH}_partition.bin
        exit 0
      else
  ...
  search_split)
    ...
    do
      for T in ${T_LIST[@]}
      do
        SEARCH_LOG=${INDEX_PREFIX_PATH}search/search_${SPLIT_TYPE}_K${K}_CACHE${CACHE}_BW${BW}_T${T}_MEML${MEM_L}_MEMK${MEM_TOPK}_PS${USE_PAGE_SEARCH}_USE_RATIO${PS_USE_RATIO}_GP_LOCK_NUMS${GP_LOCK_NUMS}_GP_CUT${GP_CUT}.log
        echo "Searching... log file: ${SEARCH_LOG}"
        case $1 in
          debug)
            sync; gdb ${EXE_PATH}/tests/search_disk_index_multi_split --ex "run --data_type $DATA_TYPE \
              --split_type $SPLIT_TYPE \
              --dist_fn $DIST_FN \
              --index_path_prefix $INDEX_PREFIX_PATH \
              --data_path $DATA_PATH \
              --pq_path_prefix $RELATIVE_PQ_PREFIX_PATH \
              --query_file $QUERY_FILE \
              --gt_file $GT_FILE \
              -K $K \
              --result_path ${INDEX_PREFIX_PATH}result/result \
              ...
              --mem_L ${MEM_L} \
              --sector_len ${SEARCH_SEC_LEN} \
              --mem_index_path ${RELATIVE_MEM_INDEX_PATH}_index \
              --mem_sample_path ${RELATIVE_MEM_SAMPLE_PATH} \
              --use_page_search ${USE_PAGE_SEARCH} \
              --use_ratio ${PS_USE_RATIO} \
              --pq_ratio ${PQ_FILTER_RATIO} \
              --disk_file_name ${DISK_FILE_NAME} \
              --graph_rep_index_prefix ${RELATIVE_GRAPH_REP_INDEX_PATH} \
              --disk_graph_prefix ${RELATIVE_GRAPH_PATH} \
              --deco_impl ${DECO_IMPL} \
              --use_graph_rep_index ${USE_DISK_GRAPH_CACHE_INDEX} \
              --mem_graph_use_ratio ${MEM_GRAPH_USE_RATIO} \
              --mem_emb_use_ratio ${MEM_EMB_USE_RATIO} \
          ...
          release)
            ...
            echo "${EXE_PATH}/tests/search_disk_index_multi_split --data_type $DATA_TYPE \
              --split_type $SPLIT_TYPE \
              --dist_fn $DIST_FN \
              ...
              --mem_emb_use_ratio ${MEM_EMB_USE_RATIO} \
              --emb_search_ratio ${EMB_SEARCH_RATIO} > ${SEARCH_LOG}"
            sync; ${EXE_PATH}/tests/search_disk_index_multi_split --data_type $DATA_TYPE \
              --split_type $SPLIT_TYPE \
              --dist_fn $DIST_FN \
              --parent_index_path $PARENT_INDEX_PATH \
              --data_path $DATA_PATH \
