...
generate_config() {
    ...
    for file in $BASE_PATH_PREFIX/$SPLIT_TYPE/_subshard-[0-9]*.bin; do
        ...
        echo "  export SECTOR_LEN=4096" >> $CONFIG_FILE
        echo "  export GR_SECTOR_LEN=4096" >> $CONFIG_FILE
        echo "  export N_PQ_CODE=4" >> $CONFIG_FILE
...
run_benchmarks() {
    ...
    for shard_id in "${shard_ids[@]}"; do
        ...
        # Run benchmark
        ./run_benchmark_multi_split.sh $INPUT_PARAMS
    done
