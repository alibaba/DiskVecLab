...
DATA_DIR=/root/paper/DiskAnnPQ/Gorgeous/data
...
print_usage_and_exit() {
  echo "Usage: ./run_benchmark.sh [debug/release] [build/build_pq/build_mem/gp/gr_layout/split_graph/search/search_io] [knn/range]"
  exit 1
}
...
case $2 in
  ...
  search_io)
    ...
    # 固定一个特大 L，专门测试不同 io_limit 对性能的影响
    SEARCH_IO_L=${SEARCH_IO_L:-1000000}

    # io_limit 列表优先使用 IO_LIMIT_LIST 数组；未配置时使用默认值
    if [ -z "${IO_LIMIT_LIST+x}" ] || [ ${#IO_LIMIT_LIST[@]} -eq 0 ]; then
      IO_LIMIT_LIST=(32 64 96 128 192 256 384 512)
      echo "IO_LIMIT_LIST is empty, using default: ${IO_LIMIT_LIST[*]}"
    fi
    ...
    do
      for T in ${T_LIST[@]}
      do
        for IO_LIMIT in ${IO_LIMIT_LIST[@]}
        do
          SEARCH_LOG=${INDEX_PREFIX_PATH}search/search_io_K${K}_L${SEARCH_IO_L}_IOLIMIT${IO_LIMIT}_CACHE${CACHE}_BW${BW}_T${T}_MEML${MEM_L}_MEMK${MEM_TOPK}_PS${USE_PAGE_SEARCH}_USE_RATIO${PS_USE_RATIO}_GP_LOCK_NUMS${GP_LOCK_NUMS}_GP_CUT${GP_CUT}.log
          echo "Searching (fixed L=${SEARCH_IO_L}, io_limit=${IO_LIMIT})... log file: ${SEARCH_LOG}"
          case $1 in
            debug)
              sync; gdb ${EXE_PATH}/tests/search_disk_index --ex "run --data_type $DATA_TYPE \
                --dist_fn $DIST_FN \
