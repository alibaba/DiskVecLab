...
print_usage_and_exit() {
  echo "Usage: ./run_benchmark_multi_split.sh [debug/release] [build/build_mem/mcrun/freq/gp/search/search_split] [knn/range]"
  exit 1
}
...
case $2 in
  ...
  ;;
  mcrun)
    MCRUN_EXE_PATH=${EXE_PATH}/my_gp
    ${MCRUN_EXE_PATH}/new_mincut ${INDEX_PREFIX_PATH} ${BASE_FPATH}

    ${EXE_PATH}/tests/utils/index_relayout ${INDEX_PREFIX_PATH}_disk.index ${INDEX_PREFIX_PATH}_partition.bin
    ...
    case $3 in
      knn)
        ...
        do
          for T in ${T_LIST[@]}
          do
            SEARCH_LOG=${PARENT_INDEX_PATH}/search_${SPLIT_TYPE}_SQ${USE_SQ}_K${K}_CACHE${CACHE}_BW${BW}_T${T}_MEML${MEM_L}_MEMK${MEM_TOPK}_MEM_USE_FREQ${MEM_USE_FREQ}_PS${USE_PAGE_SEARCH}_USE_RATIO${PS_USE_RATIO}_GP_USE_FREQ{$GP_USE_FREQ}_GP_LOCK_NUMS${GP_LOCK_NUMS}_GP_CUT${GP_CUT}.log
            echo "Searching... log file: ${SEARCH_LOG}"
