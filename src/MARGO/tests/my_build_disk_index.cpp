...
int main(int argc, char** argv) {
  ...
  if (dist_fn == std::string("l2"))
    ...
    std::cout << "Error. Only l2, mips and cosine distance functions are supported"
              << std::endl;
    return -1;
  }
  ...
  try {
    if (data_type == std::string("int8"))
      return diskann::my_build_disk_index<int8_t>(
          data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric);
    else if (data_type == std::string("uint8"))
      return diskann::my_build_disk_index<uint8_t>(
          data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric);
    else if (data_type == std::string("float"))
      return diskann::my_build_disk_index<float>(
          data_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric);
    else {
