...
// This code should only be available in the "build" DLL.
  ...
  template<typename T>
  int my_build_disk_index(const char *dataFilePath, const char *indexFilePath,
                          const char *    indexBuildParameters,
                          diskann::Metric compareMetric) {
    std::cout << "running my build disk index..." << std::endl;
    std::stringstream parser;
    parser << std::string(indexBuildParameters);
    std::string              cur_param;
    std::vector<std::string> param_list;
    ...
    size_t num_pq_chunks = atoi(param_list[2].c_str());
        // (size_t)(std::floor)(_u64(final_index_ram_limit / points_num));
...
// This code should only be available in the "build" DLL.
    ...
    diskann::my_build_merged_vamana_index<T>(
        data_file_to_use.c_str(), diskann::Metric::L2, L, R, p_val,
        indexing_ram_budget, mem_index_path, medoids_path, centroids_path);
  ...
  template DISKANN_DLLEXPORT int my_build_disk_index<int8_t>(
      const char *dataFilePath, const char *indexFilePath,
      const char *indexBuildParameters, diskann::Metric compareMetric);
  template DISKANN_DLLEXPORT int my_build_disk_index<uint8_t>(
      const char *dataFilePath, const char *indexFilePath,
      const char *indexBuildParameters, diskann::Metric compareMetric);
  template DISKANN_DLLEXPORT int my_build_disk_index<float>(
      const char *dataFilePath, const char *indexFilePath,
      const char *indexBuildParameters, diskann::Metric compareMetric);

  template<typename T>
  int my_build_merged_vamana_index(std::string     base_file,
                                   diskann::Metric compareMetric, unsigned L,
                                   unsigned R, double sampling_rate,
                                   double ram_budget, std::string mem_index_path,
                                   std::string medoids_file,
                                   std::string centroids_file) {
    std::cout << "running my build merged vamana index" << std::endl;
    size_t base_num, base_dim;
    diskann::get_bin_metadata(base_file, base_num, base_dim);
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      ...
      std::unique_ptr<diskann::myIndex<T>> _pvamanaIndex =
          std::unique_ptr<diskann::myIndex<T>>(new diskann::myIndex<T>(
              compareMetric, base_dim, base_num, false, false));
      _pvamanaIndex->mybuild(base_file.c_str(), base_num, paras);

      _pvamanaIndex->mysave(mem_index_path.c_str());
      std::remove(medoids_file.c_str());
      std::remove(centroids_file.c_str());
      return 0;
    }
    ...
    for (int p = 0; p < num_parts; p++) {
      ...
      get_bin_metadata(shard_base_file, shard_base_pts, shard_base_dim);
      std::unique_ptr<diskann::myIndex<T>> _pvamanaIndex =
          std::unique_ptr<diskann::myIndex<T>>(
              new diskann::myIndex<T>(compareMetric, shard_base_dim,
                                    shard_base_pts, false));  // TODO: Single?
      _pvamanaIndex->build(shard_base_file.c_str(), shard_base_pts, paras);
      _pvamanaIndex->mysave(shard_index_file.c_str());
      std::remove(shard_base_file.c_str());
    }
